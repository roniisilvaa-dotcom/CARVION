...
const RevenueExpenseChart = ({ data, height = 260 }) => {
  if (!data || data.length < 2) data = [{ m: '', rev: 0, exp: 0 }, { m: '', rev: 0, exp: 0 }];
  const W = 800, H = height, P = { l: 50, r: 16, t: 16, b: 28 };
  const iw = W - P.l - P.r;
  const ih = H - P.t - P.b;
  const max = Math.max(Math.max(...data.map(d => Math.max(d.rev, d.exp))) * 1.1, 1);
  const min = 0;
...
const DonutChart = ({ data, size = 180 }) => {
  if (!data || !data.length || data.reduce((s, d) => s + d.value, 0) <= 0) {
    data = [{ name: 'Sem dados', value: 1, color: 'var(--border)' }];
  }
...
const StackedBars = ({ data, keys, colors, height = 200 }) => {
  if (!data || data.length < 1) data = [{ m: '', ...Object.fromEntries(keys.map((k) => [k, 0])) }];
  const W = 460, H = height, P = { l: 36, r: 8, t: 12, b: 24 };
  const iw = W - P.l - P.r;
  const ih = H - P.t - P.b;
  const max = Math.max(Math.max(...data.map(d => keys.reduce((s, k) => s + d[k], 0))) * 1.1, 1);
...
const Heatmap = ({ grid, color = 'var(--accent)' }) => {
  if (!grid || !grid.length || !grid[0]?.length) grid = Array.from({ length: 7 }, () => Array.from({ length: 14 }, () => 0));
